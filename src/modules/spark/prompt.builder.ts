/**
 * Prompt Builder
 * Centralizes prompt templates and formatting guidelines for LLM contexts.
 */
...
export class PromptBuilder {
  /**
   * Generates the system prompt based on domain and active response mode
   */
  static buildSystemPrompt(domain: string, mode: 'scientific' | 'educational' | 'cinematic' = 'educational'): string {
    let modePersona = '';
...
`;
  ...
  /**
   * Developer guidelines/constraints that must be appended to the LLM context.
   */
  static buildDeveloperPrompt(domain: string, mode: 'scientific' | 'educational' | 'cinematic'): string {
    return `
...
`;
  ...
  /**
   * Builds the context prompt containing user history, curriculum, and knowledge base details.
   */
  static buildContextPrompt(context: LearningContext, retrievedKnowledge: IKnowledgeDocument[]): string {
  ...
  /**
   * Main formatting entry point. Combines all parts.
   */
  static formatPrompt(
    context: LearningContext,
    retrievedKnowledge: IKnowledgeDocument[],
    domain: string,
    mode: 'scientific' | 'educational' | 'cinematic'
  ): { systemPrompt: string; contextPrompt: string } {
    const systemPrompt = this.buildSystemPrompt(domain, mode);
    const developerPrompt = this.buildDeveloperPrompt(domain, mode);
    const contextPrompt = `${this.buildContextPrompt(context, retrievedKnowledge)}${developerPrompt}\nGenerate your response following these guidelines.`;
